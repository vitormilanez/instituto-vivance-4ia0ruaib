...
- Cabeçalho persistente: o chrome médico reúne título da área, contexto da rota, próxima ação e navegação principal. No topo mostra o contexto completo; ao descer entra em modo foco sem alterar a altura reservada do documento. Ao subir, restaura o contexto.
- Tese de movimento: a passagem “contexto do dia → foco” preserva título, atendimento e navegação; contexto, perfil, carteira e ações secundárias saem primeiro. Usar somente transformações, transparência e recorte do fundo em `180ms`, com listener passivo limitado por `requestAnimationFrame` e alternativa sem transição para movimento reduzido.
- Navegação adaptativa: no celular, os cinco destinos ocupam uma faixa fixa em cinco colunas logo abaixo do cabeçalho, sem rolagem lateral, com ícones compactos e rótulos ainda legíveis; no computador, a mesma faixa substitui o menu duplicado e o rail lateral fica reservado à próxima consulta e à saída.
- Encaixe dos blocos presos: abas, rail e painéis laterais devem acompanhar a altura atual do cabeçalho (`136 px` completo, `112 px` compacto) com a mesma curva de movimento, evitando sobreposição quando o contexto reaparece.
