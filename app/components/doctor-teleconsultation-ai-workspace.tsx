...
export function DoctorTeleconsultationAiWorkspace({
  ...
  if (!hydrated) {
    return (
      <section aria-labelledby="teleconsult-ai-title" className="rounded-3xl border border-[#dfe8e3] bg-white p-5 sm:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#0b7b68]">Teleconsulta demonstrativa</p>
  ...
  return (
    <section aria-labelledby="teleconsult-ai-title" className="space-y-5">
      <div className="rounded-3xl border border-[#dfe8e3] bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <AiDraftBadge>Copiloto ao vivo · simulação revisável</AiDraftBadge>
              <SessionStatus status={session.status} />
            </div>
            <h3 id="teleconsult-ai-title" className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-[#17372f]">Teleconsulta com contexto em tempo real</h3>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#60766f]">Trechos fictícios entram sob comando do médico. A IA organiza relatos, lacunas, hipóteses e divergências, sempre com tempo, fonte e revisão humana.</p>
          </div>
          <dl className="grid shrink-0 grid-cols-2 gap-px overflow-hidden rounded-2xl border border-[#dfe8e3] bg-[#dfe8e3] text-xs sm:min-w-[330px]">
            <div className="bg-[#f8faf9] p-3"><dt className="font-bold text-[#789087]">Sessão</dt><dd className="mt-1 font-bold text-[#17372f]">v{session.sessionVersion || '—'}</dd></div>
            <div className="bg-[#f8faf9] p-3"><dt className="font-bold text-[#789087]">Modo</dt><dd className="mt-1 font-bold text-[#17372f]">Mock local</dd></div>
            <div className="bg-[#f8faf9] p-3"><dt className="font-bold text-[#789087]">Trechos</dt><dd className="mt-1 font-bold text-[#17372f]">{session.visibleExcerptCount}/{transcriptScript.length}</dd></div>
            <div className="bg-[#f8faf9] p-3"><dt className="font-bold text-[#789087]">Pendentes</dt><dd className="mt-1 font-bold text-[#17372f]">{pendingCount}</dd></div>
          </dl>
        </div>
      </div>

      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(330px,0.85fr)]">
        <section aria-labelledby="teleconsult-room-title" className="overflow-hidden rounded-3xl border border-[#dfe8e3] bg-white">
          <div className="bg-[#17372f] p-5 text-white sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.11em] text-[#9fd6c8]">Slice 1 · sala e ciência</p>
                <h4 id="teleconsult-room-title" className="mt-2 text-xl font-semibold">Sala demonstrativa</h4>
              </div>
              <span className="rounded-full bg-white/10 px-3 py-1.5 text-[11px] font-bold text-[#d9eee8]">Nenhum áudio capturado</span>
            </div>
            ...
            <div className="mt-5 grid min-h-52 gap-3 sm:grid-cols-2">
              <div className="flex flex-col items-center justify-center rounded-2xl bg-white/10 p-5 text-center">
                <span className="grid size-16 place-items-center rounded-full bg-[#d9eee8] text-lg font-bold text-[#0b6a5b]">MC</span>
                <p className="mt-3 font-bold">{patientName}</p>
                <p className="mt-1 text-xs text-[#b8d3cb]">Participante fictícia</p>
              </div>
              <div className="flex flex-col items-center justify-center rounded-2xl bg-white/10 p-5 text-center">
                <span className="grid size-16 place-items-center rounded-full bg-white text-lg font-bold text-[#17372f]">GM</span>
                <p className="mt-3 font-bold">Dr. Guilherme</p>
                <p className="mt-1 text-xs text-[#b8d3cb]">Médico responsável</p>
              </div>
            </div>

            {session.status === 'idle' ? (
              <div className="mt-5 rounded-2xl bg-white p-4 text-[#17372f]">
                <label className="flex cursor-pointer items-start gap-3 text-sm leading-6">
                  <input
                    ...
                    className="mt-1 size-5 shrink-0 accent-[#0b7b68]"
                  />
                  <span><strong>Confirmo a ciência específica nesta simulação.</strong><span className="mt-1 block text-xs text-[#60766f]">Versão teleconsulta-transcricao-v1 · separada da pré-consulta e da gravação real.</span></span>
                </label>
                <button
                  type="button"
                  onClick={startSession}
                  disabled={!session.consentConfirmed}
                  className="mt-4 min-h-12 w-full cursor-pointer rounded-xl bg-[#0b7b68] px-4 text-sm font-bold text-white transition-colors hover:bg-[#096b5b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8fd3c0] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-[#8a9c96]"
                >
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {session.status === 'active' ? (
                  <button type="button" onClick={() => setSessionStatus('paused')} className="min-h-11 cursor-pointer rounded-xl bg-white px-4 text-sm font-bold text-[#17372f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8fd3c0] focus-visible:ring-offset-2 focus-visible:ring-offset-[#17372f]">Pausar assistência</button>
                ) : session.status === 'paused' ? (
                  <button type="button" onClick={() => setSessionStatus('active')} className="min-h-11 cursor-pointer rounded-xl bg-white px-4 text-sm font-bold text-[#17372f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8fd3c0] focus-visible:ring-offset-2 focus-visible:ring-offset-[#17372f]">Retomar assistência</button>
                ) : null}
                {session.status !== 'ended' ? (
                  <button type="button" onClick={() => setSessionStatus('ended')} className="min-h-11 cursor-pointer rounded-xl border border-white/25 px-4 text-sm font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#17372f]">Encerrar sessão</button>
                ) : (
                  <button type="button" onClick={resetSession} className="min-h-11 cursor-pointer rounded-xl bg-white px-4 text-sm font-bold text-[#17372f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8fd3c0] focus-visible:ring-offset-2 focus-visible:ring-offset-[#17372f]">Preparar nova simulação</button>
                )}
              </div>
            )}
          </div>
          ...
          <div className="p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.1em] text-[#0b7b68]">Transcrição incremental</p>
                <h4 className="mt-2 text-lg font-semibold text-[#17372f]">Trechos de origem preservados</h4>
              </div>
              {latestExcerpt ? <span className="text-xs font-bold text-[#789087]">Último trecho · {latestExcerpt.at}</span> : null}
            </div>
            ...
            {visibleExcerpts.length > 0 ? (
              <ol className="mt-4 max-h-[420px] space-y-3 overflow-y-auto pr-1" aria-label="Transcrição demonstrativa da teleconsulta">
                {visibleExcerpts.map((excerpt) => (
                  <li key={excerpt.id} className={cn('rounded-2xl border p-4', excerpt.speaker === 'Paciente' ? 'border-[#c9d8ec] bg-[#f7faff]' : 'border-[#d7e3df] bg-[#f8faf9]')}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <strong className="text-xs text-[#405d54]">{excerpt.speaker}</strong>
                      <time className="text-[11px] font-bold text-[#789087]">{excerpt.at} · {excerpt.id}</time>
                    </div>
                    <p className="mt-2 text-sm leading-6 text-[#526a62]">{excerpt.body}</p>
                  </li>
                ))}
              </ol>
            ) : (
              <p className="mt-4 rounded-2xl border border-dashed border-[#bfd4cd] bg-[#fbfdfc] p-5 text-sm leading-6 text-[#60766f]">A transcrição permanece vazia até a ciência específica e o início explícito da simulação.</p>
            )}
        ...
        <aside aria-labelledby="live-insights-title" className="h-fit rounded-3xl border border-[#c9d8ec] bg-[#f7faff] p-5 sm:p-6 xl:sticky xl:top-24">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.1em] text-[#456b9c]">Slice 2 · IA em tempo real</p>
              <h4 id="live-insights-title" className="mt-2 text-lg font-semibold text-[#17372f]">Insights que exigem decisão</h4>
            </div>
            <Status tone={pendingCount > 0 ? 'amber' : 'gray'}>{pendingCount} pendentes</Status>
          </div>
          <p className="mt-2 text-xs leading-5 text-[#60766f]">Cada cartão aponta o trecho exato. Fixar não aprova diagnóstico, conduta ou mensagem.</p>
          ...
          {visibleInsights.length > 0 ? (
            <div className="mt-4 max-h-[760px] space-y-3 overflow-y-auto pr-1">
              {visibleInsights.map((insight) => {
                ...
                return (
                  <article key={insight.id} className={cn('rounded-2xl border bg-white p-4', review.decision === 'pinned' ? 'border-[#8bbcaf]' : review.decision === 'dismissed' ? 'border-[#d7e3df] opacity-80' : 'border-[#dce6f2]')}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <ClinicalLayerBadge layer={presentation.layer} />
                        <Status tone={presentation.tone}>{presentation.label}</Status>
                      </div>
                      <span className="text-[11px] font-bold text-[#789087]">{source?.at} · {insight.coverage === 'direct' ? 'fonte direta' : 'cobertura parcial'}</span>
                    </div>
                    <h5 className="mt-3 text-sm font-bold leading-6 text-[#17372f]">{insight.title}</h5>
                    <p className="mt-1 text-xs leading-5 text-[#60766f]">{insight.detail}</p>
                    <blockquote className="mt-3 border-l-2 border-[#9eb9dd] pl-3 text-xs italic leading-5 text-[#526a62]">{insight.sourceQuote}</blockquote>
                    <p className="mt-2 break-all text-[10px] font-semibold text-[#789087]">Fonte: {insight.excerptId}</p>
                    {resolved ? <p className="mt-3 rounded-xl bg-[#edf7f4] p-3 text-xs font-semibold leading-5 text-[#0b6a5b]">Lacuna respondida no trecho {insight.resolvedByExcerptId}; confirme se ainda merece a pauta.</p> : null}
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2" role="group" aria-label={`Decisão sobre ${insight.title}`}>
                      <button
                        ...
                        className={cn('min-h-11 cursor-pointer rounded-xl border px-3 text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2', review.decision === 'pinned' ? 'border-[#0b7b68] bg-[#0b7b68] text-white' : 'border-[#9ccdc2] text-[#0b6a5b] hover:bg-[#edf7f4]')}
                      >
                        Fixar
                      </button>
                      <button
                        ...
                        className={cn('min-h-11 cursor-pointer rounded-xl border px-3 text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2', review.decision === 'dismissed' ? 'border-[#8a9c96] bg-[#f1f5f3] text-[#405d54]' : 'border-[#d7e3df] text-[#60766f] hover:bg-[#f4f7f5]')}
                      >
                        Descartar
                      </button>
                    </div>
                    ...
                    {review.decision === 'dismissed' ? (
                      <label className="mt-3 block text-xs font-bold text-[#405d54]">
                        Motivo do descarte
                        <select
                          value={review.dismissalReason ?? ''}
                          onChange={(event) => setDismissalReason(insight.id, event.target.value || null)}
                          className="mt-2 min-h-11 w-full rounded-xl border border-[#d7e3df] bg-white px-3 text-sm font-normal text-[#405d54] outline-none focus:ring-2 focus:ring-[#8bc6b9]"
                        >
                    ...
                    <details className="mt-3 rounded-xl border border-[#e2e9e6] bg-[#f8faf9] p-3">
                      <summary className="cursor-pointer text-xs font-bold text-[#405d54] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68]">Ver limite do insight</summary>
                      <p className="mt-2 text-xs leading-5 text-[#60766f]">{insight.limitation}</p>
                    </details>
                  </article>
                );
              })}
            ...
            <p className="mt-4 rounded-2xl border border-dashed border-[#c9d8ec] bg-white p-5 text-sm leading-6 text-[#60766f]">Os insights aparecem somente quando um trecho fictício contém informação organizável.</p>
          )}
        </aside>
      </div>

      <section aria-labelledby="teleconsult-closing-title" className="rounded-3xl border border-[#dfe8e3] bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-[#0b7b68]">Slice 3 · fechamento versionado</p>
            <h4 id="teleconsult-closing-title" className="mt-2 text-xl font-semibold text-[#17372f]">Dos itens fixados para as notas médicas</h4>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#60766f]">O rascunho separa relatos, pontos a confirmar e hipóteses. Só entra nas notas depois de edição e aprovação explícita.</p>
          </div>
          <Status tone={session.draftStatus === 'approved' ? 'green' : session.draftStatus === 'rejected' ? 'gray' : session.draftStatus === 'draft' ? 'amber' : 'gray'}>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
          <div>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm font-bold text-[#17372f]">Rascunho assistido</p>
              <button
                type="button"
                onClick={prepareDraft}
                disabled={!canBuildDraft}
                className="min-h-11 cursor-pointer rounded-xl border border-[#9ccdc2] px-4 text-xs font-bold text-[#0b6a5b] hover:bg-[#edf7f4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:border-[#d7e3df] disabled:text-[#8a9c96] disabled:hover:bg-white"
              >
            ...
            <textarea
              ...
              className="mt-3 min-h-72 w-full rounded-2xl border border-[#d7e3df] bg-[#fbfdfc] p-4 font-mono text-xs leading-6 text-[#405d54] outline-none focus:ring-2 focus:ring-[#8bc6b9]"
            />
            <div className="mt-3 flex flex-wrap gap-2">
              {session.draftStatus === 'approved' ? (
                <>
                  <button type="button" onClick={reapplyApprovedDraft} className="min-h-11 cursor-pointer rounded-xl bg-[#0b7b68] px-4 text-sm font-bold text-white hover:bg-[#096b5b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2">Reaplicar versão aprovada</button>
                  <button type="button" onClick={() => setSession((current) => ({ ...current, draftStatus: 'draft' }))} className="min-h-11 cursor-pointer rounded-xl border border-[#bfd4cd] px-4 text-sm font-bold text-[#0b6a5b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2">Criar nova versão</button>
                </>
              ) : (
                <button
                  type="button"
            ...
            {session.draftStatus === 'rejected' && session.draftRejectionReason ? <p className="mt-3 rounded-xl bg-[#f1f5f3] p-3 text-xs leading-5 text-[#526a62]">Motivo registrado: {session.draftRejectionReason}. O conteúdo permanece preservado somente nesta sessão demonstrativa.</p> : null}
          </div>

          <div>
            <label htmlFor="teleconsult-notes" className="text-sm font-bold text-[#17372f]">Notas médicas</label>
            <textarea
              id="teleconsult-notes"
              value={notes}
              onChange={(event) => onNotesChange(event.target.value)}
              className="mt-3 min-h-72 w-full rounded-2xl border border-[#d7e3df] p-4 text-sm leading-6 text-[#405d54] outline-none focus:ring-2 focus:ring-[#8bc6b9]"
            />
            <p className="mt-3 text-xs leading-5 text-[#789087]">Este campo continua sob autoria médica. Aplicar o rascunho não publica, prescreve nem sincroniza prontuário.</p>
            <button type="button" onClick={onContinue} className="mt-4 min-h-12 w-full cursor-pointer rounded-xl bg-[#17372f] px-4 text-sm font-bold text-white hover:bg-[#24483e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68] focus-visible:ring-offset-2">Construir próximo plano</button>
          </div>
        </div>

        <details className="mt-5 rounded-2xl border border-[#dfe8e3] bg-[#f8faf9] p-4">
          <summary className="cursor-pointer text-sm font-bold text-[#405d54] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7b68]">Auditoria da sessão demonstrativa</summary>
          {session.auditEvents.length > 0 ? (
            <ol className="mt-4 space-y-2">
              {[...session.auditEvents].reverse().map((event) => <li key={event.id} className="flex flex-col gap-1 rounded-xl bg-white p-3 text-xs sm:flex-row sm:items-start sm:justify-between"><span className="leading-5 text-[#526a62]">{event.label}</span><time className="shrink-0 font-bold text-[#789087]">{event.at}</time></li>)}
            </ol>
          ) : <p className="mt-3 text-xs leading-5 text-[#60766f]">Nenhuma transição registrada nesta sessão.</p>}
          <p className="mt-3 text-[11px] leading-5 text-[#789087]">A auditoria guarda transições e contagens, não o conteúdo clínico original.</p>
        </details>
      </section>

      <p aria-live="polite" className={cn('rounded-2xl border p-4 text-sm font-semibold', feedback ? 'border-[#b9d8cf] bg-[#edf7f4] text-[#0b6a5b]' : 'border-[#dfe8e3] bg-[#f8faf9] text-[#60766f]')}>
